...
- **`openaiVideoProvider.js`** — real adapter for OpenAI's Sora Videos API (`POST/GET https://api.openai.com/v1/videos`, `GET /v1/videos/{id}/content?variant=video`). Supports image-to-video via `input_reference` (multipart) when an input image is attached to the job. Reads `VIDEO_PROVIDER_API_KEY` or falls back to `OPENAI_API_KEY` — **the app boots and runs fine without either set**; the error only surfaces (503) if a job actually tries to use the `openai` provider with no key configured.
  ...
  **Gotcha confirmed in production**: OpenAI's `input_reference` requires the image to be **exactly** the target output's pixel dimensions (`Inpaint image must match the requested width and height` — a live 400 we hit). Ordinary product photos never happen to match. `GET /video-jobs/config` exposes `aspectRatioToOpenAiSize` (aspect ratio + model → exact `WxH`) specifically so `SCAiGenVid` can center-crop-resize the uploaded image client-side (`src/lib/imageResize.js`, canvas-based) to that exact size before upload. If you build a second frontend against this API, you must replicate that resize step or image-to-video will fail for every real upload.
